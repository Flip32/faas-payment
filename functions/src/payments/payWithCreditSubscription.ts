...
export const payWithCreditSubscription = async (data: any, context: any) => {
    ...
    const idempotencyKey = getIdempotencyKey('credit-subscription', customerInfo.id, `${planId}-${periodicity}`)

    const response = await axios.post('https://api.efi.com.br/v1/subscription', {
